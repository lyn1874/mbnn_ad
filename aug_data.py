...
parser.add_argument('--train_or_test', type=str, help="training or testing")
...
#     path2write="/project_scratch/bo/anomaly_data/Avenue_play/Avenue/frames/%s" % use_str
    ...
    for iterr, single_child in enumerate(path_child):
        ...
        while (cap.isOpened()):
            ...
            if ret == False:
                break
            cv2.imwrite(path2write + '/%s_video_%d_frame_%d.jpg' % (use_str, int(s_video_name), i), frame)
            i += 1
        cap.release()
...
def save_avenue_rain_or_bright(data_path, rain_type, aug, train_or_test, bright_space=0.2):
    ...
    """
    if "Avenue" in data_path:
        data_augment_path = data_path + 'frames/%s_%s/' % (rain_type, train_or_test)
    else:
        data_augment_path = data_path + 'Avenue/frames/%s_%s/' % (rain_type, train_or_test)
    create_folder(data_augment_path)
    print("Augment data into path:", data_augment_path)
    tr, tt, imshape, targshape = read_avenue_data(data_path)
    im_use = [tr if train_or_test == "training" else tt][0]
    time_step = [1 if rain_type != "original" else 10][0]
    ...
    if bright_space == 1.0 and rain_type == "original":
        aug = False
    if "train" in train_or_test:
        if aug == True:
            bright_space = [bright_space / 10.0 if bright_space > 1 else bright_space][0]
            bright_space = [bright_space]
        else:
            bright_space = [1.0]
    else:
        bright_space = [bright_space / 10.0 if bright_space > 1 else bright_space]
    print("Adjusting the illumination of the images to", bright_space)
...
def read_avenue_data(model_mom):
    tr_tot = []
    if "Avenue" in model_mom:
        path_mom = model_mom + "frames/"
    else:
...
#         save_avenue_frame("training", args.datapath)
        ...
        save_avenue_rain_or_bright(args.datapath, args.rain_type, True, args.train_or_test, bright_space=args.bright)
